<?php
    ...
    if (!isset($_SESSION['username'])) {
      header("Location: ../HW06/loginCrud/login.php");
      exit();
    };
?>
<!DOCTYPE html>
...
<section class="vh-100">
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST'){
       ...
       $server->reservation($conn,$tableName,$dayAmount,$price,$isMember,$name,$email,$peopleAmount);

        
    }
    ?></h4>
     <button type="submit" class="btn btn-primary btn-lg" style="padding-left: 2.5rem;  margin-top:50px; padding-right: 2.5rem;"><a href='index.php'>กลับหน้าหลัก</a></button>
        </div>
        </div>
    </div>
    </section>
